...
plot(cif2,ylim=c(0,0.40),xlab="Time (days)",ylab="Cumulative incidence", 
     ...
     main=c(paste("True CIF Plot - Simulated Data Controls"), paste("n=100,000 Patients (Scenario 4)")))
legend(x="topleft",c("Control PCa CIF: 9.8%", "Control OD CIF: 28.4%"),
       col=c("blue", "blue"),lwd=2,lty=c(1,2),bty="n", c=0.8)


###combined true graphs
plot(cif1,ylim=c(0,0.50),xlab="Time (days)",ylab="Cumulative incidence", 
     col=c("red","red"),lwd=2,lty=c(1,2),conf.int=F,
     main=c(paste("True CIF Plot - Simulated Data"), paste("n=100,000 Patients (Scenario 4)")))
lines(cif2,col=c("blue","blue"),lwd=2,lty=c(1,2),conf.int=F)

legend(x="topleft",c("Treated PCa CIF: 7.9%","Control PCa CIF: 9.8%",
                     "Treated OD CIF: 28.9%", "Control OD CIF: 28.4%"),
