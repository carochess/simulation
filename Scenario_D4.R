...
generate_cif_data <- function() {
  ...
  #obtain treatment time which depends on L. We need 51.2% of the patients 
  ##to start treatment within 6 months to compare with the RWD
  #The higher the shape the higher the risk of experiencing the event 
  #soon after time 0. The higher the scale the longer the survival times.
  ...
  eventT20<-rexp(n, rate = lambda2 * exp(betaL2*L))
  hist(eventT20)
  eT2<-eventT20
  hist(eT2)
  
  
  # Generate censoring times: censoring=0%
  # on average, a very small % of the patients will be censored per unit time
  ...
  # Create a data frame
  Simdata <- data.frame(Patientid = 1:n,
                        L=L,
                        A=A,
                        Treatment_time=v,
  ...
  plot(km, col=c("blue","red"), fun="event", xlab="Time (days)",
       ...
       lwd=2,conf.int=F,main=c(paste("Complement of the KM Plot - Simulated Data"), 
                               paste("n=500 Patients (Scenario 4) 1 replicate"))
  )
  
  legend(x="topleft",c("Treatment strategy A=0: 42%","Treatment strategy A=1: 33%"),
  ...
  plot(cif,ylim=c(0,0.40),xlab="Time (days)",ylab="Cumulative incidence", 
       col=c("blue","red", "blue", "red"),lwd=2,lty=c(1,1,2,2),conf.int=F,
       main=c(paste("CIF Plot - Simulated Data"), paste("n=500 Patients (Scenario 4) 1 replicate")))
  legend(x="topleft",c("Treated PCa CIF: 7.8%","Control PCa CIF: 10.8%", 
                       "Treated OD CIF: 25.0%", "Control OD CIF: 31.4%"),
  ...
  table(Simdata$rt6)
  ##256 received treatment within 6 months
  ...
  data.long_t6$prostateDeath<- data.long_t6$outcome==1 & data.long_t6$Event==1
  data.long_t6$otherDeath<- data.long_t6$outcome==1 & data.long_t6$Event==2
  ...
  ##LET'S CREATE A COLUMN FOR PROSTATE DEATH AND OTHER DEATH
  data.long_c6$prostateDeath<- data.long_c6$outcome==1 & data.long_c6$Event==1
  data.long_c6$otherDeath<- data.long_c6$outcome==1 & data.long_c6$Event==2
  ...
  data_final$compet<-ifelse(data_final$outcome==1 & data_final$Event==1, 1, 0)
  data_final$compet<-ifelse(data_final$outcome==1 & data_final$Event==2, 
                            2, data_final$compet)
  ...
  Sim2<-Simdata_treatment[Simdata_treatment$fup>=182.62,]
  
  
  ms_cens_t<-glm(censoring~ L+I(L^2), data=Sim2, family = binomial)
  summary(ms_cens_t) 
  ...
  ms_cens_tnum<-glm(censoring~ 1 , data=Sim2, family = binomial)
  ...
  data.long_c6$Lsq<-data.long_c6$L*data.long_c6$L
  ##Cox model
  #  ms_cens_c6<-coxph(Surv(Tstart, Tstop, censoring)~L+Lsq, ties = "efron",
  # data= data.long_c6)
  ...
  #data.long_c6$hazard[is.na(data.long_c6$hazard)]<-0
  
  beta<-coef(ms_cens_c6)
  # data.long_c6$lin_pred<- as.matrix(data.long_c6[,c(4,21)])%*%beta
...
final_df4 <- do.call(cbind, cif_dfs)


### For the average rowmeans we have
# Initialize an empty list to store data frames
average_cif_dfs4 <- list()

# Calculate row-wise means for each CIF
for (i in 1:8) {
  # Extract CIFs for the current CIF number
  cif_cols <- grep(paste("CIF", i, sep = "_"), colnames(final_df4))
  cif_data <- final_df4[, cif_cols]
  
  # Calculate row-wise means
  average_cif_dfs4[[i]] <- rowMeans(cif_data, na.rm = TRUE)
}

# Combine the data frames into one data frame
average_df4 <- do.call(cbind, average_cif_dfs4)

# Set column names for the average data frame
colnames(average_df4) <- paste("Average_CIF", 1:8, sep = "_")
average_df4 <- as.data.frame(average_df4)

# Save the data frame as a CSV file
write.csv(final_df4, "final_cif_data_scenario4.csv", row.names = FALSE)
write.csv(average_df4, "average_cif_data_scenario4.csv", row.names = FALSE)

#average_df4<-average_cif_data_scenario4
#final_df4<-final_cif_data_scenario4

##Plot the cif's for nsim=100 and n=500
library(dbplyr)
library(tidyverse)

average_df4 <- rename(average_df4, "Average PCa treated*" = Average_CIF_1,
...
average_df4$Time<-seq(0,7,0.1)
df_long <- gather(average_df4, key = "Variables", value = "Values", -Time)
...
p1 <- ggplot(df_subset1, aes(x = Time, y = Values, linetype = Variables, color = Variables)) +
  ...
  scale_y_continuous(limits = c(0, 60)) +  # Set limits for the y-axis
  scale_x_continuous(breaks = seq(0, 7, by = 1)) +
  labs(color = "Variables", title = "Risk Estimates Controlling for L Using CCW Approach \nScenario 4: n=500 patients and 100 replicates")+
  geom_text_repel(data = last_p1, aes(label = sprintf("%.1f", Values)), vjust = ifelse(last_p1$nudge_y == -1, -0.01, 0.501), direction = "y", size = 2, show.legend = FALSE)  # Add text labels with exact values at the end, suppressing the legend
...
p2 <- ggplot(df_subset2, aes(x = Time, y = Values, linetype = Variables, color = Variables)) +
  ...
  scale_y_continuous(limits = c(0, 60)) +  # Set limits for the y-axis
  scale_x_continuous(breaks = seq(0, 7, by = 1)) +
  labs(color = "Variables", title = "Risk Estimates Ignoring L Using Standard Approach \nScenario 4: n=500 patients and 100 replicates")+
...
allaverage<-cbind(average_df4, truetreat, truecontrol)
