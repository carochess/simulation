...
generate_cif_data <- function() {
  ...
  plot(km, col=c("blue","red"), fun="event", xlab="Time (days)",
       ...
       lwd=2,conf.int=F,main=c(paste("Complement of the KM Plot - Simulated Data"), 
                               paste("n=500 Patients (Scenario 3) 1 replicate"))
  )
  
  legend(x="topleft",c("Treatment strategy A=0: 37%","Treatment strategy A=1: 27%"),
  ...
  plot(cif,ylim=c(0,0.40),xlab="Time (days)",ylab="Cumulative incidence", 
       ...
       main=c(paste("CIF Plot - Simulated Data"), paste("n=500 Patients (Scenario 3) 1 replicate")))
  legend(x="topleft",c("Treated PCa CIF: 7.8%","Control PCa CIF: 19.6%", 
                       "Treated OD CIF: 19.7%", "Control OD CIF: 18.2%"),
  ...
  ms_cens_t<-glm(censoring~ L, data=Sim2, family = binomial)
  summary(ms_cens_t) 
...
final_df3 <- do.call(cbind, cif_dfs)


### For the average rowmeans we have
# Initialize an empty list to store data frames
average_cif_dfs3 <- list()

# Calculate row-wise means for each CIF
for (i in 1:8) {
  # Extract CIFs for the current CIF number
  cif_cols <- grep(paste("CIF", i, sep = "_"), colnames(final_df3))
  cif_data <- final_df3[, cif_cols]
  
  # Calculate row-wise means
  average_cif_dfs3[[i]] <- rowMeans(cif_data, na.rm = TRUE)
}

# Combine the data frames into one data frame
average_df3 <- do.call(cbind, average_cif_dfs3)

# Set column names for the average data frame
colnames(average_df3) <- paste("Average_CIF", 1:8, sep = "_")
average_df3 <- as.data.frame(average_df3)

# Save the data frame as a CSV file
write.csv(final_df3, "final_cif_data_scenario3.csv", row.names = FALSE)
write.csv(average_df3, "average_cif_data_scenario3.csv", row.names = FALSE)

#average_df3<-average_cif_data_scenario3
#final_df3<-final_cif_data_scenario3

##Plot the cif's for nsim=100 and n=500
library(dbplyr)
library(tidyverse)

average_df3 <- rename(average_df3, "Average PCa treated*" = Average_CIF_1,
...
average_df3$Time<-seq(0,7,0.1)
df_long <- gather(average_df3, key = "Variables", value = "Values", -Time)
...
p1 <- ggplot(df_subset1, aes(x = Time, y = Values, linetype = Variables, color = Variables)) +
  ...
  scale_y_continuous(limits = c(0, 50)) +  # Set limits for the y-axis
  scale_x_continuous(breaks = seq(0, 7, by = 1)) +
  labs(color = "Variables", title = "Risk Estimates Controlling for L Using CCW Approach \nScenario 3: n=500 patients and 100 replicates")+
...
allaverage<-cbind(average_df3, truetreat, truecontrol)
