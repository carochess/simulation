...
generate_cif_data <- function() {
  ...
  #obtain treatment time which depends on L. We need approximately 51.2% of the patients 
  ...
  eventT10<-rexp(n, rate = lambda1 * exp(betaL*L))
  hist(eventT10)
  
  newT1<-rexp(n, rate = lambda1 * exp(betaL*L + betaA1))
  ...
  # Generate censoring times: censoring=2.5%
  ...
  plot(km, col=c("blue","red"), fun="event", xlab="Time (days)",
       ylim=c(0,0.4), ylab="1-Survival probability",
       lwd=2,conf.int=F,main=c(paste("Complement of the KM Plot - Simulated Data"), 
                               paste("n=500 Patients (Scenario 2) 1 replicate"))
  )
  
  legend(x="topleft",c("Treatment strategy A=0: 41%","Treatment strategy A=1: 37%"),
  ...
  plot(cif,ylim=c(0,0.4),xlab="Time (days)",ylab="Cumulative incidence", 
       ...
       main=c(paste("CIF Plot - Simulated Data"), paste("n=500 Patients (Scenario 2) 1 replicate")))
  legend(x="topleft",c("Treated PCa CIF: 9.4%","Control PCa CIF: 13.6%", 
                       "Treated OD CIF: 27.3%", "Control OD CIF: 27.2%"),
  ...
  ms_cens_t_poly<-glm(censoring~ L+I(L^2) , data=Simdata_treatment, 
                      family = binomial)
  summary(ms_cens_t_poly) 
  
  ms_cens_t_poly3<-glm(censoring~ L+I(L^2)+I(L^3) , data=Simdata_treatment, family = binomial)
  summary(ms_cens_t_poly3) 
  
  # Compare AIC values
  AIC(ms_cens_t, ms_cens_t_poly, ms_cens_t_poly3)
  # Compare BIC values
  BIC(ms_cens_t, ms_cens_t_poly, ms_cens_t_poly3)
  
  newdatat$prob<-predict(ms_cens_t_poly,newdata=newdatat, type="response")
  ...
  data.long_t6<-data.long_t6[order(data.long_t6$timeID, data.long_t6$Tstart),]
...
final_df2 <- do.call(cbind, cif_dfs)


### For the average rowmeans we have
# Initialize an empty list to store data frames
average_cif_dfs2 <- list()

# Calculate row-wise means for each CIF
for (i in 1:8) {
  # Extract CIFs for the current CIF number
  cif_cols <- grep(paste("CIF", i, sep = "_"), colnames(final_df2))
  cif_data <- final_df2[, cif_cols]
  
  # Calculate row-wise means
  average_cif_dfs2[[i]] <- rowMeans(cif_data, na.rm = TRUE)
}

# Combine the data frames into one data frame
average_df2 <- do.call(cbind, average_cif_dfs2)

# Set column names for the average data frame
colnames(average_df2) <- paste("Average_CIF", 1:8, sep = "_")
average_df2 <- as.data.frame(average_df2)

# Save the data frame as a CSV file
write.csv(final_df2, "final_cif_data_scenario2.csv", row.names = FALSE)
write.csv(average_df2, "average_cif_data_scenario2.csv", row.names = FALSE)

#average_df2<-average_cif_data_scenario2
#final_df2<-final_cif_data_scenario2

##Plot the cif's for nsim=100 and n=500
library(dbplyr)
library(tidyverse)

average_df2 <- rename(average_df2, "Average PCa treated*" = Average_CIF_1,
...
average_df2$Time<-seq(0,7,0.1)
df_long <- gather(average_df2, key = "Variables", value = "Values", -Time)
...
allaverage<-cbind(average_df2, truetreat, truecontrol)
